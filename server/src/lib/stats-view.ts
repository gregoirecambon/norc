// Historical statistics: SQL aggregates over task_runs — no telemetry
// pipeline, no per-event tables. Cheap by construction: task_runs is capped at
// 90 days by pruneOldRuns(), so every query here scans a small table. Token
// numbers are agent-self-reported (best-effort) — the payload carries
// runsWithTokens so consumers can label the coverage honestly. Shared by the
// dashboard route (/api/stats) and the machine surface (/api/ext/stats).

import { sql } from 'drizzle-orm';
import { db } from '../db/client.js';
import { notionIntegration } from '../db/schema.js';
import { userDisplayName } from './notion-anchor.js';
import { slackUserName } from './slack-client.js';
import { getSlack, isSlackActive } from './slack-integration.js';

export const STATS_WINDOWS = [7, 30, 90];

export async function buildStats(days: number): Promise<Record<string, unknown>> {
